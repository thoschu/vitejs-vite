...
export function average(...args: Array<number>): number {
  ...
  const sum: number = args.reduce(
    (acc: number, val: number): number => add(acc, val),
    0
  );
...
export function median(...args: Array<number>): number {
  ...
  const sorted: Array<number> = [...args].sort((a: number, b: number): number =>
    subtract(a, b)
  );
...
export function validateUuid(uuid: string, version: number): boolean {
  return and<boolean, boolean>(
    uuidValidate(uuid),
    equals<number>(uuidVersion(uuid), version)
  );
}
